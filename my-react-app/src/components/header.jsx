...
const Header = ({ onLoginOpen, onRegisterOpen }) => (
    <header class="fixed inset-x-20 top-2 z-50 rounded-full border-transparent glass-nav">
        <nav class="mx-auto flex h-20 max-w-7xl items-center justify-between px-6 lg:px-8">
            ...
            {/* --- DESKTOP MENU: Cuma muncul kalau isLoggedIn bernilai TRUE --- */}
            {isLoggedIn && (
            ...
            <div className="hidden items-center gap-3 md:flex">
                {!isLoggedIn ? (
                    ...
                    <>
                        <button data-action="Login" className="action-btn rounded-full px-4 py-2.5 text-sm font-bold text-forest transition hover:bg-white">Login</button>
                        <button data-action="Register" className="action-btn rounded-full bg-gold px-5 py-2.5 text-sm font-black text-forest shadow-lg shadow-gold/25 transition hover:-translate-y-0.5 hover:bg-[#e2b454]">Register</button>
                    </>
                ) : (
                    // Kalau SUDAH login, ganti tombolnya (Misal jadi Profile atau Logout)
                    // Silakan disesuaikan dengan kebutuhan aplikasimu nanti
                    <button data-action="Logout" className="action-btn rounded-full px-4 py-2.5 text-sm font-bold text-red-500 transition hover:bg-white">Logout</button>
                )}
            </div>
